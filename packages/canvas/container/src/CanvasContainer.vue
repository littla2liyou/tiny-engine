...
import { useTranslate, useCanvas, useMessage, useResource, useNotify } from '@opentiny/tiny-engine-meta-register'
  ...
  setup(props, { emit }) {
    ...
    onMounted(() => {
      ...
      const handleCanvasPageSwitch = (event) => {
        if (canvasApi.validatePageSwitchMessage(event)) {
          const { pageId } = event.data
          canvasApi.handleCanvasPageSwitch(pageId).catch((_error) => {
            useNotify({
              type: 'error',
              message: '页面跳转失败'
            })
